...
export class Server extends Endpoint {
  constructor(opts) {
    ...
    this.opts = opts || {};
    this.opts.type =
      typeof this.opts.type !== "undefined" ? this.opts.type : "http";
    this.opts.headers = this.opts.headers || {};
    this.opts.websocket =
      typeof this.opts.websocket !== "undefined" ? this.opts.websocket : true;
  }
      ...
      if (!this.authHandler(username, password)) {
        if (res) {
          ...
          Server.handleHttpError(
            req,
            res,
            new Error.InvalidParams(UNAUTHORIZED),
            self.opts.headers
          );
        }
        return false;
  ...
  listen(port, host) {
    ...
    if (port) {
      ...
      Endpoint.trace(
        "***",
        "Server listening on http://" + (host || "127.0.0.1") + ":" + port + "/"
      );
    }
  ...
  listenRaw(port, host) {
    ...
    Endpoint.trace(
      "***",
      "Server listening on tcp://" + (host || "127.0.0.1") + ":" + port + "/"
    );

    return server;
  }

  listenHybrid(port, host) {
    var self = this,
    ...
    Endpoint.trace(
      "***",
      "Server (hybrid) listening on http+tcp://" +
        (host || "127.0.0.1") +
        ":" +
        port +
        "/"
  ...
  handleHttp(req, res) {
    ...
    if (req.method !== "POST") {
      Server.handleHttpError(
        req,
        res,
        new Error.InvalidRequest(METHOD_NOT_ALLOWED),
        self.opts.headers
      );
      return;
    }
    ...
    var handle = function handle(buf) {
      ...
      try {
        ...
        Server.handleHttpError(
          req,
          res,
          new Error.ParseError(INVALID_REQUEST),
          self.opts.headers
        );
        return;
      }
      ...
      if (!decoded.method || !decoded.params) {
        ...
        Server.handleHttpError(
          req,
          res,
          new Error.InvalidRequest(INVALID_REQUEST),
          self.opts.headers
        );
        return;
      }
      ...
      var callback = function callback(err, result) {
        ...
        if (err) {
          ...
          Endpoint.trace(
            "-->",
            "Response (id " + decoded.id + "): " + JSON.stringify(result)
          );
  ...
  handleWebsocket(request, socket, body) {
    var self = this,
      conn,
      parser;
  ...
  handleHybrid(httpServer, socket) {
    var self = this;
  ...
  /**
     ...
     */
  static handleHttpError(req, res, error, custom_headers) {
